...
solver.add_assertion(And([And(GE(var[v], Int(1)), LE(var[v], Int(9))) for v in var.keys()]))

# Each col has distinct values
for c in range(5):
  solver.add_assertion(AllDifferent(list(map(lambda p : p[1], filter(lambda p : int(p[0][1]) == c, var.items())))))

# Each row has distinct values
for r in range(6):
  solver.add_assertion(AllDifferent(list(map(lambda p : p[1], filter(lambda p : int(p[0][2]) == r, var.items())))))
